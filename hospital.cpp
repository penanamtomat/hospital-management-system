...
bool CompareByName(Hospital& A, Hospital& B) {
    return A.H_name < B.H_name;
}

// Function to sort the hospital data by name
void SortHospitalByName(vector<Hospital>& hospitals) {
    sort(hospitals.begin(), hospitals.end(), CompareByName);
    cout << "SORT BY NAME:" << endl << endl;
    PrintHospitalData(hospitals);
}

// Comparator function to sort the hospital data by rating
bool CompareByRating(Hospital& A, Hospital& B) {
    return A.rating > B.rating;
}

// Function to sort the hospital data by rating
void SortHospitalByRating(vector<Hospital>& hospitals) {
    sort(hospitals.begin(), hospitals.end(), CompareByRating);
    cout << "SORT BY Rating:" << endl << endl;
    PrintHospitalData(hospitals);
}

// Comparator function to sort the hospital data by available beds
bool CompareByBeds(Hospital& A, Hospital& B) {
    return A.available_beds > B.available_beds;
}

// Function to sort the hospital data by available beds
void SortByBedsAvailable(vector<Hospital>& hospitals) {
    sort(hospitals.begin(), hospitals.end(), CompareByBeds);
